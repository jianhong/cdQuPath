...
### Merge multiple Seurat object if necessary

If you have dataset from multiple sub-regions, you can process all of them as it shown above and then merge them.
For some reasons, some images may not have same background in different regions.
You can simply detect the cells for each sub-regions with similar background or baseline,
and then do GMM rescaled for each of them.
After that, you can merge them by combining the analized results.
...
seu$orig.ident <- 'region2'
mergedSeu <- merge(seu, seu2, merge.dr = TRUE)
mergedSeu
Assays(mergedSeu)
...
FeaturePlot(mergedSeu, slot='count',
            ...
            reduction = 'pos',
            ncol = 2)
DimPlot(mergedSeu, group.by = 'celltype', reduction = 'pos')
## use GMM rescaled data to plot the correlation
