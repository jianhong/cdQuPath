...
ggplot(plotData, aes(x="", y = percent, fill = label)) +
  ...
  geom_text(aes(y=y_pos, label=round(percent*100, digits = 2)), color = 'white') +
  coord_polar(theta = 'y') +
  facet_wrap(~grouping) +
  theme_void() +
...
Here is the sample code to create the classifier for your own data and
align the single cell RNA-seq data to the Codex data.
```{r alignCells}
...
This method are based on simple yet intuitive measures, such as counting the number of nearest neighbors of every cell and inspecting the proportion of different cell categories in a given distance.

If you increase the maxgap, you should recalculate the nearest neighbor by increasing the numbers ob neighbors to hold all possible neighbors.
...
The co-occurrence score gives us an indication on whether given two factors such as cell cluster, cell type, and etc, co-occur with each other at increasing distances across the tissue. The co-occurrence score is defined as: $\frac{p(factor1|factor2)}{p(factor1)p(factor2)}$.
...
spe <- neighborhoodsAna(mergedSeu, clusterK = clusterK,
